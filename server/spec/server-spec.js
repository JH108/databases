...
describe('Persistent Node Chat Server', function() {
  ...
  it('Should insert posted messages to the DB', function(done) {
    ...
    request({
      ...
      //console.log('checkpoint #1: users post ran beautifully');
      // so... these functions who shall not be named... how are they being called
  ...
  it('Should output all messages from the DB', function(done) {
    ...
    var queryString = 'INSERT INTO messages(content, userId, roomname) VALUES (?,?,?)';
  ...
  it('Should output all users from the DB', function(done) {
    ...
    var queryArgs = ['Javert'];

    dbConnection.query(queryString, queryArgs, function(err) {
      if (err) { throw err; }

      // Now query the Node chat server and see if it returns
      // the user we just inserted:
      request('http://127.0.0.1:3000/classes/users', function(error, response, body) {
        var userLog = JSON.parse(body);
        expect(userLog[userLog.length - 1].username).to.equal('Javert');
        done();
      });
    });
  });
